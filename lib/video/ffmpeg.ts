...
function ffmpegDataToBlobPart(data: Uint8Array | string): BlobPart {
  ...
  // DO NOT use new Uint8Array(data) or copy.set(data) here.
  // Windows Chrome memory fragmentation can cause crashes if we double the allocation size.
  // Passing the view directly to the Blob constructor is safe and memory-efficient.
  return data as any;
}
...
async function mountInputFile(
  ...
  try {
    ...
    log.error('WORKERFS mount failed', err, { sizeMb });
    throw new Error(
      `Could not mount video file (WorkerFS failed). Windows often restricts memory access for large files. ` +
        `Try restarting your browser or closing other tabs to free up memory contiguity.`,
    );
  }
}
