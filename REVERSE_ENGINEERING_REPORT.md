...
## 14. Windows-Specific Troubleshooting & Optimization

During reverse engineering, several Windows-specific crash vectors were identified and mitigated to ensure stability on memory-fragmented systems.

### 14.1 Memory Fragmentation Mitigation
- **Direct Blob Creation:** The FFmpeg read-back logic was optimized to avoid "memory doubling." Instead of copying WASM memory into a new `Uint8Array` before creating a `Blob`, the WASM view is passed directly. This reduces the peak memory requirement for a 200MB chunk from 400MB to 200MB.
- **Binary IPC Transfer:** The bridge between the side panel and the content script was updated to use direct **binary `Uint8Array`** instead of standard JS Arrays. Converting binary data to a JS Array expands the memory footprint by 8x (as every byte becomes a 64-bit float). Using direct binary transfer keeps the memory usage at a 1:1 ratio.

### 14.2 Out-of-Memory (OOM) Prevention
- **WORKERFS Enforcement:** On Windows, falling back to a full memory copy of a 1GB+ video file (the "heap fallback") almost always results in a tab crash. The extension now enforces `WORKERFS` for mounting. If the browser cannot mount the file without a copy, it throws a user-friendly error instead of attempting a crash-prone memory allocation.
- **Contiguous Allocation:** By reducing concurrent memory-heavy tasks, the extension maximizes the chance of finding a contiguous block of virtual address space, which is more scarce on Windows than on macOS.
